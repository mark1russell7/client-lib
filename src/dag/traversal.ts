 ...
 */
export function buildLeveledDAG(nodes: Map<string, DAGNode>): DependencyDAG {
  // Step 1: Compute in-degrees (number of dependencies each node has within the DAG)
  const inDegree = new Map<string, number>();
  const dependents = new Map<string, Set<string>>(); // reverse edges
 ...
 */
export function getTopologicalOrder(dag: DependencyDAG): DAGNode[] {
 ...
 */
export function visualizeDAG(dag: DependencyDAG): string {
  const lines: string[] = ["DAG Structure:"];
