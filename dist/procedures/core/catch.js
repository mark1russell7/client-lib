 ...
 * ```
 */
import { isAnyProcedureRef, normalizeRef, hydrateInput, } from "@mark1russell7/client";
/**
 * Execute a procedure with error handling
 */
export async function coreCatch(input, ctx) {
    ...
    try {
        ...
        try {
            const decision = await ctx.client.call(normalizedHandler.path, hydratedHandlerInput);
            return {
                success: false,
                error: errorMessage,
                result: decision,
